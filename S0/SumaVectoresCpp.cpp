...
int main(int argc, char** argv){ 
  ...
  struct timespec cgt1,cgt2;   //para tiempo de ejecucion
  ...
  double *v1, *v2, *v3;
  v1 = new double [N];    //si no hay espacio suficiente new genera una excepcion
  v2 = new double [N];
  v3 = new double [N];
  #endif
 
  //Inicializar vectores 
  for(int i=0; i<N; i++){ v1[i] = N*0.1+i*0.1; v2[i] = N*0.1-i*0.1; //los valores dependen de N
  } 
  ...
  #ifdef COUT_ALL
  cout << "Tiempo(seg.):" << ncgt << "\t/ Tamano Vectores:" << N << endl;
  for(int i=0; i<N; i++) 
      cout << "/ V1[" << i << "]+V2[" << i << "]=V3" << i << "](" << v1[i] << "+" << v2[i] << "=" 
      << v3[i] << ") /\t" << endl; 
  cout <<"\n"<< endl;
  #else
      cout << "Tiempo(seg.):" << ncgt << "\t/ Tamano Vectores:" << N << "\t/ V1[0]+V2[0]=V3[0](" 
